...
public class LoadBalanceSocket implements Runnable {
	...
	public void run() {
		...
		System.out.println("The server number : "+serverNumber);
		
		LoadBalanceListener.serverSelector = (LoadBalanceListener.serverSelector + 1) % LoadBalance.servers.size();
		
		System.out.println("The server selected : " +LoadBalance.servers.get(serverNumber));
		
		
		this.selectedServer = LoadBalance.servers.get(serverNumber);
		System.out.println("The Server Used : " +selectedServer.getHost());
		System.out.println("The Port Used : " +selectedServer.getPort());
