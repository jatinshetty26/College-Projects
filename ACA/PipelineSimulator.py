...
class PipelineSimulator(object): 
    ...
    def __init__(self,instrCollection):
        print("Initializing clocks, counters and other stuff")
        self.instrCount = 0
        self.cycles = 0
        self.hazardList = []
        self.__done = False
        ...
        print("Adding stages to pipeline : ",self.pipeline)
        ...
        # ex: {'$r0' : 0, '$r1' : 0 ... '$r31' : 0 }
        print("Creating 32bit registers")
        ...
        print("self.registers : ", self.FPregisters)


        print("set up the main memory construct, a list index starting at 0 and continuing to 0xffc")
        print("This is basically a range of ",int(0xffc/4))

        self.mainmemory = dict([(x*4, 0) for x in range(int(0xffc/4))])
        print("self.mainmemory : ",self.mainmemory)


        print("programCounter to state where in the instruction collection we are. to find correct spot in mainmemory add 0x100 ")
        self.programCounter = 0x1000
        print("self.programCounter : ",self.programCounter)

        print("the list of instruction objects passed into the simulator, most likely created by parsing text")
        self.instrCollection = instrCollection
        print("self.instrCollection : ",self.instrCollection)
       
        print("populate main memory with our text of the instructions starting at 0x100")
        y=0
        ...
        print("Initialize iCache")
        self.initializeICache()
        self.initializeDCache()



    def initializeICache(self):
        print("Start building Instruction Cache")
        # code goes here
        self.iCache = {}
        print("Ending Cache initialization")


    def initializeDCache(self):
        print("Start building Instruction Cache")
        # code goes here
        self.dCache = {}
        print("Ending Cache initialization")
    ...
    def step(self):
        ...
        print("shift the instructions to the next logical place. Technically we do the Fetch instruction here, which is why FetchStage.advance() does nothing")
        
        #MUST KEEP THIS ORDER

        self.pipeline[1] = WriteStage(self.pipeline[4].instr,self)

        print("Check if stall : ",self.stall)
        ...
        print("call advance on each instruction in the pipeline")
        for pi in self.pipeline:
            pi.advance()
        print("now that everything is done, remove the register from the hazard list")
        if (self.pipeline[1].instr.regWrite) :
            print("Removing from hazardList")
            self.hazardList.pop(0)
        
        self.checkDone()
        print("if we stalled our branch we dont want to load a new, so keep the program counter where it is")
    ...
    def checkDone(self):
        print("Check if we are done and set __done variable")
        self.__done = True
        for pi in self.pipeline:
            print("The PI Instruction : ",pi.instr)
            if pi.instr is not Nop:
                self.__done = False
    
    def run(self):
        print("Run the simulator, call step until we are done")
        while not self.__done:
            self.step()
            self.debug()
    
    def getForwardVal(self, regName): #r3
...
class FetchStage(PipelineStage):
    def advance(self):
        print("Fetch the next instruction according to simulator program counter")
        ...
        if self.simulator.programCounter < (len(self.simulator.instrCollection) * 4 + 0x1000):
            ...
            print("Attempting to read from iCache")
...
class ReadStage(PipelineStage):
    ...
    def advance(self):
        print("Start - Read the necessary registers from the registers file used in this instruction")

        if(self.instr.regRead):
            if '$f' in self.instr.s1:
                print("The FP register")
                self.instr.source1RegValue = self.simulator.FPregisters[self.instr.s1]
            else:
                print("The R register")
            ...
            if (self.instr.immed and
                ...
                print("check to see if it is a hex value")
        ...
        print("End 1.0 - Read from Registers")
                    
        if self.instr.op == 'j':
            print("Set the program counter to the raw target address")
            ...
            print("Set the other instructions currently in the pipeline to a Nop")
            self.simulator.pipeline[0] = FetchStage(Nop, self)
        print("End 1.1 - Read from Registers")
...
class ExecStage(PipelineStage):
    def advance(self):
        print("Start - Execute the instruction according to its mapping of assembly operation to Python operation")
        ...
        if self.instr is not Nop and self.instr.aluop:
            ...
            print("append the destination register to the hazard list")
            if self.instr.regWrite :
                self.simulator.hazardList.append(self.instr.dest)    

            print("calculate the offset of the lw and sw instructions")
            if  self.instr.op == 'lw':
...
class DataStage(PipelineStage):
    def advance(self):
        print("If we have to write to main memory, write it first and then read from main memory second")
...
class WriteStage(PipelineStage):
    def advance(self):
        print("Starting - Write to the register file")
